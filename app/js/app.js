        ...
        var grid = $('#bootgrid-command').bootgrid({
            formatters: {
                commands: function(column, row) {
                    return '<button type="button" class="btn btn-sm btn-info mr-2 command-edit" onclick="window.location.href=\'chamado.php\'" data-row-id="' + row.id + '"><em class="fa fa-edit fa-fw"></em></button>' +
                ...
                commandEditUser: function(column, row) {
                    return '<button type="button" class="btn btn-sm btn-info mr-2 command-edit-user" data-row-id="' + row.id + '"><em class="fa fa-edit fa-fw"></em></button>' +
                        '<button type="button" class="btn btn-sm btn-danger command-delete-user" data-row-id="' + row.id + '"><em class="fa fa-trash fa-fw"></em></button>';
                },
                commandEditLancamento: function(column, row) {
                    return '<button type="button" class="btn btn-sm btn-info mr-2 command-edit-lancamento" data-row-id="' + row.id + '"><em class="fa fa-edit fa-fw"></em></button>' +
                        '<button type="button" class="btn btn-sm btn-danger command-delete-lancamento" data-row-id="' + row.id + '"><em class="fa fa-trash fa-fw"></em></button>';
                }
            },
            labels: bootgridlabels,
            templates: {
                // templates for BS4
                actionButton: '<button class="btn btn-secondary" type="button" title="{{ctx.text}}">{{ctx.content}}</button>',
                actionDropDown: '<div class="{{css.dropDownMenu}}"><button class="btn btn-secondary dropdown-toggle dropdown-toggle-nocaret" type="button" data-toggle="dropdown"><span class="{{css.dropDownMenuText}}">{{ctx.content}}</span></button><ul class="{{css.dropDownMenuItems}}" role="menu"></ul></div>',
            ...
            grid.find('.command-edit').on('click', function() {
                $('#modal-interacao').modal('show');
            }).end().find('.command-delete').on('click', function() {
                console.log('You pressed delete on row: ' + $(this).data('row-id'));
            });

            grid.find('.command-edit-user').on('click', function() {
                $('#modal-usuario').modal('show');
            }).end().find('.command-delete-user').on('click', function() {
            });

            grid.find('.command-edit-lancamento').on('click', function() {
                $('#modal-lancamento').modal('show');
            }).end().find('.command-delete-lancamento').on('click', function() {
            });
